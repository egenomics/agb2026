...
def plot_module1_diversity(diversity_df, pass_mask, threshold, out_dir):
    ...
    fig.suptitle(f"Alpha diversity split by retention (threshold = {threshold:,} reads)", fontsize=11)
    fig.tight_layout()
    fig.savefig(out_dir / "diversity_by_retention.png", dpi=150)
    plt.close(fig)
    log.info("Saved diversity_by_retention.png")
...
def main():
    ...
    individual_pngs = [
        ...
        out_dir / "diversity_by_retention.png",
    ]
    ...
    if existing:
        ...
        # Add extra space only between the first and second images
        if len(imgs) >= 3:
            gap = 0.03  
