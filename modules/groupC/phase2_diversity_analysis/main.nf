...
include {DIVERSITY_METRICS} from './modules/module_diversity_metrics/diversity_analysis.nf'
include {RAREFACTION_THRESHOLD;
        SKIP_RAREFACTION} from './modules/module_rarefaction/select_rarefaction_batches.nf'
...
workflow {
    ...
    // Debug: log key outputs to console
    PHASE2_DIVERSITY.out.sampling_depth.view       { "Sampling depth         : ${it.text.trim()}" }
    PHASE2_DIVERSITY.out.depth_used.view           { "Rarefaction depth used : ${it[1].text.trim()}" }
    PHASE2_DIVERSITY.out.rarefaction_plots.view    { "Rarefaction plots      : ${it[1]}" }
    PHASE2_DIVERSITY.out.skip_report.view          { "Skip report            : ${it[1]}" }
}