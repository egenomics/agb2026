...
def _slope_traces(rows: list[dict[str, Any]], metric: str) -> list[dict[str, Any]]:
    ...
    for p in get_unique_patients(rows):
        p_rows = sorted([r for r in rows if r["patient"] == p], key=lambda r: r.get("time") or 0)
        if len(p_rows) < 2:
            continue
    ...
    return [
        {
            ...
            "marker": {
                "color": [
                    hex_rgba(base_group_color(s["base_group"], base_groups), 0.8) for s in scores
                ]
            },
...
def _rank_traces(rows: list[dict[str, Any]], metric: str) -> list[dict[str, Any]]:
    ...
    for bg in base_groups:
        c = base_group_color(bg, base_groups)
        pts = [
            (i, r) for i, r in enumerate(sorted_rows) if r.get("base_group", r.get("group")) == bg
        ]
                ...
                round(
                    float(
                        np.mean(
                            [
                                float(r.get(t) or 0)
                                / (sum(float(r.get(tt) or 0) for tt in taxa) or 1)
                                * 100
                                for r in t0s
                            ]
                        )
