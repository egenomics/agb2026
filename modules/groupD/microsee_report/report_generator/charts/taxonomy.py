...
def build_donut(
    ...
    for gi, g in enumerate(groups):
        ...
        traces.append({
            ...
            "labels": list(taxa),
            "values": [
                float(np.mean([float(r.get(t) or 0) for r in g_rows]))
                for t in taxa
            ],
...
def build_taxonomy_views(
    rows: list[dict[str, Any]], taxa: list[str], base_groups: list[str],
) -> dict[str, Any]:
    ...
    for tp_key, tp_val in tp_map.items():
        for grp_key, grp_val in grp_map.items():
            ...
            if grp_val:
                filtered = [
                    r for r in filtered
                    if r.get("base_group", r.get("group")) == grp_val
                ]
            if not filtered:
                continue
            for topn in topns:
...
def build_sunburst(
    ...
    for g in groups:
        g_rows     = [r for r in rows if r["group"] == g]
        child_vals = [
            round(float(np.mean([float(r.get(t) or 0) for r in g_rows])), 1)
            for t in taxa
        ]
