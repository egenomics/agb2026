...
def _explain_comparative(chart_data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    # Largest LFC hit per direction from diff_abundance traces
...
def _explain_clinical(
    ...
    for r in rows:
        p = r["patient"]
        tp = r.get("timepoint", "")
        mwt_v = float(r.get("sixmwt") or 0)
        il18_v = float(r.get("il18") or 0)
        if mwt_v > 0:
...
def _explain_longitudinal(chart_data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for tr in long_traces:
        ...
        if "T0" in xs and "T84" in xs:
            delta = float(ys[xs.index("T84")]) - float(ys[xs.index("T0")])
            long_changes.append((str(tr.get("name", "")), delta))
    ...
    return {
        "longitudinal": {
            ...
            "pills": ([f"{g}: {d:+.3f}" for g, d in long_changes[:2]] + ["Shannon H′"])
            if long_changes
