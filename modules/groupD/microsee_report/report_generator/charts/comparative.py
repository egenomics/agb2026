...
def build_diff_abundance(rows: list[dict[str, Any]], taxa: list[str]) -> list[dict[str, Any]]:
    ...
    for bg in _base_groups(rows):
        ...
        t84_rows = [
            r for r in rows if r.get("base_group", r["group"]) == bg and r.get("timepoint") == "T84"
        ]
        ...
        lfc_vals = [round(math.log2((m84 + _PSEUDO) / (m0 + _PSEUDO)), 3) for _, m0, m84 in stats]
        traces.append(
            {
                "type": "bar",
                "orientation": "h",
...
def build_volcano(rows: list[dict[str, Any]], taxa: list[str]) -> list[dict[str, Any]]:
    ...
    for bg in bgs:
        ...
        for t in taxa:
            ...
            lfc = round(
                math.log2((float(np.mean(b_vals)) + _PSEUDO) / (float(np.mean(a_vals)) + _PSEUDO)),
                3,
            )
...
def build_ancom_style(rows: list[dict[str, Any]], taxa: list[str]) -> list[dict[str, Any]]:
    ...
    for bg in bgs:
        ...
        results = [(t, diff, p, q) for (t, diff, p), q in zip(taxon_stats, q_vals, strict=False)]
        traces.append(
            {
                "type": "bar",
                "orientation": "h",
    ...
    return [
        {
            ...
            "y": list(taxa),
            "z": [
                [round(float(corr[i, j]), 3) for j in range(len(taxa))] for i in range(len(taxa))
            ],
            "colorscale": "RdBu",
            "zmid": 0,
            "zmin": -1,
