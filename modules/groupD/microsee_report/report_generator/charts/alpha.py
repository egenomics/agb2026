...
def _bracket_data(rows: list[dict[str, Any]], groups: list[str], base_groups: list[str],
    ...
    def _bracket(x0: int, x1: int, y_b: float, label: str, dashed: bool = False) -> None:
        ...
        shapes.extend([
            {"type": "line", "x0": x0, "x1": x1, "y0": y_b, "y1": y_b,
             "xref": "x", "yref": "y", "line": {"color": col, "width": 1, "dash": ls}},
            {"type": "line", "x0": x0, "x1": x0, "y0": y_b - tick, "y1": y_b,
             "xref": "x", "yref": "y", "line": {"color": col, "width": 1}},
            {"type": "line", "x0": x1, "x1": x1, "y0": y_b - tick, "y1": y_b,
             "xref": "x", "yref": "y", "line": {"color": col, "width": 1}},
        ])
        annots.append({"x": (x0 + x1) / 2, "y": y_b + y_span * 0.025,
                        "xref": "x", "yref": "y", "text": label,
...
def build_rarefaction(rows: list[dict[str, Any]], taxa: list[str], groups: list[str]) -> list[dict[str, Any]]:
    ...
    for g in groups:
        ...
        traces.append({
            "type": "scatter", "mode": "lines", "x": depths, "y": upper,
            "line": {"width": 0}, "showlegend": False, "hoverinfo": "skip", "name": g,
        })
        # Lower boundary fills up to upper
        traces.append({
            "type": "scatter", "mode": "lines", "x": depths, "y": lower,
            "fill": "tonexty", "fillcolor": hex_rgba(c, 0.15),
            "line": {"width": 0}, "showlegend": False, "hoverinfo": "skip", "name": g,
        })
        # Mean line
        traces.append({
            "type": "scatter", "mode": "lines", "name": g, "x": depths, "y": mean_v,
            "line": {"color": c, "width": 2.5},
            "hovertemplate": (f"<b>{g}</b><br>Depth: %{{x}}<br>"
