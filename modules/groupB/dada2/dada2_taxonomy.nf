process DADA2_TAXONOMY {
    ...
    # Species-level exact-match against the SILVA species reference.
    # assignTaxonomy with a toGenus train set leaves Species as NA; addSpecies fills it
    # where the ASV sequence matches a known species exactly (allowMultiple=TRUE returns
    # comma-joined names for ambiguous matches).
    taxa\$tax <- addSpecies(taxa\$tax, \"$species_database\", allowMultiple = TRUE, tryRC = TRUE)

    # Sync the bootstrap matrix: addSpecies is exact-match, so confidence is 100% where
    # a species was assigned, 0 otherwise. Without this the downstream confidence column
    # would silently index past the boot matrix's last column for species-assigned ASVs.
    if (!(\"Species\" %in% colnames(taxa\$boot))) {
        taxa\$boot <- cbind(taxa\$boot, Species = 0)
    }
    taxa\$boot[, \"Species\"] <- ifelse(is.na(taxa\$tax[, \"Species\"]), 0, 100)

    cat(\"addSpecies: assigned species to\", sum(!is.na(taxa\$tax[, \"Species\"])),
        \"of\", nrow(taxa\$tax), \"ASVs\\n\")
